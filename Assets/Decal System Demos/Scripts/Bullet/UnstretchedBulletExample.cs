...
namespace Edelweiss.DecalSystem.Example {
	...
	public class UnstretchedBulletExample : MonoBehaviour {
		...
		[SerializeField] private float m_DecalProjectorOffset = 0.5f;
			...
			// create at runtime.
		[SerializeField] private Vector3 m_DecalProjectorScale = new Vector3 (2f, 2f, 2f);
		[SerializeField] private float m_CullingAngle = 80.0f;
		[SerializeField] private float m_MeshOffset = 2.0f;
		...
		private void Update () {
			...
			if (check) {
				...
				if (Physics.Raycast (l_Ray, out l_RaycastHit, Mathf.Infinity)) {
					...
					Vector3 l_ProjectorPosition = changePos - (m_DecalProjectorOffset * l_Ray.direction.normalized);
