			...
			if (Input.GetButtonDown ("Fire1")) {
				...
				if (Physics.Raycast (l_Ray, out l_RaycastHit, Mathf.Infinity)) {
					...
					if (l_RaycastHit.rigidbody != null) {
							...
							// We only perform projections here, if we haven't hit a rigidbody.
							// If we are dealing with a rigidbody, we pass the projection information
							// to the dynamic object.
						BulletExampleDynamicObject l_BulletExampleDynamicObject = l_RaycastHit.collider.GetComponent <BulletExampleDynamicObject> ();
